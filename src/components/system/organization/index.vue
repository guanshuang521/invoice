<template>
  <div class="dashboard-editor-container add-edit-info-container">
    <el-form class="form" ref="form" :model="userInfo" label-width="100px" :rules="rules">
      <el-form-item label="任务名称：" prop="title" >
        <el-input v-model="userInfo.title" placeholder="长度不超过50"></el-input>
      </el-form-item>
      ...
      <el-form-item style="margin-top: 0;margin-bottom: 12px!important;line-height:16px" v-if="userInfo.mode == 'auto'">
        <p style="font-size: 12px;color: #747474;margin: 0">设置预约执行时间，系统自动导入号码，到期后自动执行任务</p>
      </el-form-item>
      <el-form-item style="margin-top: 0;margin-bottom: 12px!important;line-height:16px" v-if="userInfo.mode == 'manually'">
        <p style="font-size: 12px;color: #747474;margin: 0">任务创建者自己导入号码，需手动启动任务方可执行</p>
      </el-form-item>
      <el-form-item label="执行时间：" prop="startTime" v-if="isStartTimeShow">
        <el-date-picker v-model="userInfo.startTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期" :picker-options="pickerOptions0"></el-date-picker>
      </el-form-item>
      <el-form-item label="描述：" prop="description">
        <el-input type="textarea" v-model="userInfo.description" rows="4" resize="none" placeholder="长度不超过200"></el-input>
      </el-form-item>
    </el-form>
  </div>
</template>
